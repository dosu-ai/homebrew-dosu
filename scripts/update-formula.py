...
def render_formula(version: str, resolved_assets):
    values = {}
...
class Dosu < Formula
  ...
  on_macos do
    on_intel do
      url "https://github.com/dosu-ai/dosu-cli/releases/download/v#{{DOSU_VERSION}}/{values['DARWIN_X86_NAME']}"
      sha256 "{values['DARWIN_X86_SHA']}"
    end
    on_arm do
      url "https://github.com/dosu-ai/dosu-cli/releases/download/v#{{DOSU_VERSION}}/{values['DARWIN_ARM_NAME']}"
  ...
  on_linux do
    on_intel do
      url "https://github.com/dosu-ai/dosu-cli/releases/download/v#{{DOSU_VERSION}}/{values['LINUX_X86_NAME']}"
      sha256 "{values['LINUX_X86_SHA']}"
    end
    on_arm do
      url "https://github.com/dosu-ai/dosu-cli/releases/download/v#{{DOSU_VERSION}}/{values['LINUX_ARM_NAME']}"
...
def main():
    if len(sys.argv) != 3:
        raise SystemExit("Usage: update-formula.py <version> <formula_file>")

    version = sys.argv[1]
    formula_file = sys.argv[2]

    resolved_assets = resolve_release_assets(version)
    formula = render_formula(version, resolved_assets)

    with open(formula_file, "w", encoding="utf-8") as handle:
        handle.write(formula)

    for _key, label, asset_name, checksum in resolved_assets:
